---
name: recall
description: Retrieve the codebase-engineering learnings that apply to the files you are about to work in, scoped by path and phase. Use at the start of planning or implementation instead of reading whole learnings files. Trigger on "/recall", "recall learnings", or before touching an unfamiliar area.
allowed_tools: ["Bash"]
---
...
The learnings directory is the project's `docs/learnings` unless configured otherwise.
...
2. **Pick the phase.** Planning a change → `--phase planning`. Writing code →
   `--phase impl`. Omit to get both.

3. **Run recall:**

   ```bash
   "${LEARNINGS_CLI:-node ~/Documents/learnings-db/src/cli.mjs}" recall \
     --dir docs/learnings \
...
4. **Use the returned bullets** as context for the plan/implementation. The
